...
const METRIC_INFO = {
  aid: {
    ...
    changed: 'We do not change Lowy\'s spent/disbursed aid values. We show spent/disbursed aid only, not commitments, because it is the money that actually flowed. Aggregate donor categories are removed, so the table shows direct donor relationships only.',
  ...
  remittances: {
    ...
    changed: 'We do not change the published modelled bilateral estimates. The source provides benchmark years only, so we do not interpolate missing years or re-estimate missing flows. Non-country aggregate rows are removed where present.',
    denominator: 'GDP uses [World Bank] where available; [UN SNAAMA] for Cook Islands; [Niue Statistics Office National Accounts] for Niue.',
    denominatorLinks: DENOMINATOR_SOURCES.gdp,
  },
  migration: {
    ...
    changed: 'We do not change the UN migrant stock values. Regional and aggregate destination rows are removed so the table shows country-to-country destinations only.',
  ...
  debt: {
    ...
    changed: 'We do not change IDS debt stock values. We show the rows where IDS reports that a Pacific country owed money to a creditor in that year, and keep the creditor names IDS reports, including multilateral and institutional creditors.',
    denominator: 'GDP uses [World Bank] where available; [UN SNAAMA] for Cook Islands; [Niue Statistics Office National Accounts] for Niue.',
    denominatorLinks: DENOMINATOR_SOURCES.gdp,
  },
}
