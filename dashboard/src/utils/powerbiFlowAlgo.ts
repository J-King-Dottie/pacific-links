/*
 * Adapted from PowerBI-Flowmap by Weiwei Cui.
 * Source: https://github.com/weiweicui/PowerBI-Flowmap
 *
 * MIT License
 * Copyright (c) 2017 Weiwei Cui
 */

type StringMap<T> = { [key: string]: T };
type Func<T, R = void> = (value: T, index?: number) => R;

function extent<T>(arr: T[], selector: (v: T) => number): [number, number] {
    let min = Infinity;
    let max = -Infinity;
